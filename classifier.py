...
def run_experiment(df, n_round=35, test_size=0.3, random_state=42,
    ...
    for model_name, model in tqdm(models.items(), desc="Modelos"):
        for rnd in tqdm(range(n_round), desc=f"{model_name} rounds", leave=False):
            ...
            if train_type == "hold_out":
                print(f"Treinando modelo {model_name} por meio de hold-out, round {rnd}...")
                ...
                if tuning == "random":
                    print("Executando busca otimização com random search...")
                    ...
                    print("Executando busca otimização com bayesian search...")
                    ...
                    print(f"Treinando modelo {model_name} com parâmetros padrão...")
                ...
                print(f"Treinando modelo {model_name} de cross-validation, round {rnd}...")
                from sklearn.model_selection import StratifiedKFold
                skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=random_state + rnd)
                ...
                if tuning == "random":
                    print("Executando busca otimização com random search...")
                    search = RandomizedSearchCV(pipe, search_spaces.get(model_name, {}),
                                                n_iter=5, cv=skf, scoring="f1", n_jobs=-1)
                    #print("Treinando o modelo...")
                    search.fit(X, y)
                    best_model = search.best_estimator_
                    best_params = {k.replace("clf__", ""): v for k, v in search.best_params_.items()}
                ...
                temp = pd.DataFrame({
                    ...
                    "tuning": tuning,
                    "rep_type": rep_type,
                    "best_params": [json.dumps(best_params, ensure_ascii=False)] * len(y)
                })
                results.append(temp)

    results_df = pd.concat(results).reset_index(drop=True)

    # expandir hiperparâmetros em colunas (mantém best_params JSON também)
    try:
        expanded = results_df["best_params"].apply(json.loads).apply(pd.Series)
        results_df = pd.concat([results_df, expanded], axis=1)
    except Exception:
        pass
    ...
    for model_name in results_df.model.unique():
        for ttype in results_df.train_type.unique():
            for tune in results_df.tuning.unique():
                for rep in results_df.rep_type.unique():
                    sub = results_df[(results_df.model == model_name) &
                                     (results_df.train_type == ttype) &
                                     (results_df.tuning == tune) &
                                     (results_df.rep_type == rep)]
                    if len(sub) > 0:
                        # pega params da primeira linha
                        params_dict = json.loads(sub["best_params"].iloc[0])
                        ...
                        metrics.append({
                            ...
                            "tuning": tune,
                            "rep_type": rep,
                            "accuracy": accuracy_score(sub["true_class"], sub["pred_class"]),
                            "precision": precision_score(sub["true_class"], sub["pred_class"]),
                            "recall": recall_score(sub["true_class"], sub["pred_class"]),
                            "f1": f1_score(sub["true_class"], sub["pred_class"]),
                            "roc_auc": roc_auc_score(sub["true_class"], sub["proba_pred"]),
                            "best_params": json.dumps(params_dict, ensure_ascii=False)
                        })

    metrics_df = pd.DataFrame(metrics)
    
    print(f"\n✅ Experimento finalizado | {len(results_df)} previsões geradas | {len(metrics_df)} métricas agregadas\n")
...
for exp_idx, (tuning, train_type, rep_type) in enumerate(all_combinations, start=1):
    ...
    try:
        results_df, metrics_df = run_experiment(df, **params_)

        # nomes padronizados dos arquivos
        results_path = os.path.join(output_dir, f"results_df_{tuning}_{train_type}_{rep_type}.parquet")
        metrics_path = os.path.join(output_dir, f"metrics_df_{tuning}_{train_type}_{rep_type}.parquet")
